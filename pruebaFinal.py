    ...
    with open(archivo_csv, 'r', newline='') as file:
        ...
        for row in reader:
            if len(row) == 4:
                ...
                print(f"Error: Fila incorrecta - {row}")
...
def comprar_producto():
    ...
    if not articulo_existente:
        ...
        nuevo_id_cliente = input("Ingrese su IDCLIENTE para la compra: ")
        print(f"Compra exitosa. Producto disponible para la compra. IDCLIENTE asignado: {nuevo_id_cliente}")
        asignar_cliente_a_articulo(id_articulo_a_comprar, nuevo_id_cliente)
    else:
    ...
    with open(archivo_csv, 'r', newline='') as file:
        ...
        for row in reader:
            if row[0] == id_articulo:
                # Se encontró el IDARTICULO, devolver el IDCLIENTE actual
                return True, row[3] if row[3] else None  # Devolver el IDCLIENTE o None si no está asignado
...
def agregar_articulo():
    ...
    with open(archivo_csv, 'a', newline='') as file:
        ...
        writer.writerow([id_articulo, serie, sucursal, id_cliente])
